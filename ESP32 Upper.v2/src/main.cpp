...
//サーボ複数同時制御
//目標角度1～10、角度変更速度
void moveServosToTarget(int targetAngle1, int targetAngle2, int targetAngle3, int targetAngle4, int targetAngle5, int targetAngle6, int targetAngle7, int targetAngle8, int targetAngle9, int targetAngle10, int delayTime) {
  ...
  int currentAngle6 = myservo6.read();
  ...
  if (Serial.available()) { //シリアル読みとり
    ...
    if (firstDelimiterIndex > 0){ //コマンド判定
      ...
      if (secondDelimiterIndex > 0) { //数値個数判定
        ...
        if (command == "CB"){ //A
          if (firstValue == 1){
            moveServosToTarget(180,90,180,100,90,90,90,90,90,90,stepDelay); //サーボ角度制御
          }
        }
        if (command == "TB"){ //X
          if (firstValue == 1){
            moveServosToTarget(180,90,160,55,30,0,0,0,0,0,stepDelay);
            moveServosToTarget(135,90,160,55,30,0,0,0,0,0,stepDelay);
            moveServosToTarget(135,90,180,100,90,0,0,0,0,0,stepDelay);
          }
        }
        if (command == "XB"){ //B
          if (firstValue == 1){
            moveServosToTarget(180,180,180,0,110,180,180,180,180,180,stepDelay);
          }
        }
        if (command == "SB"){ //Y
          if (firstValue == 1){
            moveServosToTarget(135,180,10,120,70,0,0,0,0,0,stepDelay);
            moveServosToTarget(180,180,10,120,70,0,0,0,0,0,stepDelay);
          }
        }
        if (command == "R2A"){ 
          if (firstValue == 1){
            moveServosToTarget(180,90,170,58,38,180,180,180,180,180,stepDelay);
          }
        }
        if (command == "R1B"){ 
          if (firstValue == 1){
        ...
        if (command == "PB"){ 
          if (firstValue == 1){
            moveServosToTarget(180,90,180,90,38,180,180,180,180,180,stepDelay);
          }
        }
        if (command == "KS"){
          if(firstValue == 1){
