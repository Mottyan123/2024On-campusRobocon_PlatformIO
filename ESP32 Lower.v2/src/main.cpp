...
//モータ設定
CytronMD motor1(PWM_DIR, 18, 19); //FR （R…後、R…右）
CytronMD motor2(PWM_DIR, 32, 33); //FL
CytronMD motor3(PWM_DIR, 25, 26); //RL
CytronMD motor4(PWM_DIR, 16, 17); //RR

const float wheelRadius = 0.03;  // タイヤの半径(m)
const float robotRadius = 0.1;   // マシンの中央からタイヤまでの距離(m)
const int settingspeed = 130; //モーターの最大スピード
