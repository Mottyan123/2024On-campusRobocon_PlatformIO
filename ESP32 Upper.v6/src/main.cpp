...
const int servo_angle1 = 90; //180; //サーボ初期角度（電源ON時に初期化）
const int servo_angle2 = 90; //180;
const int servo_angle3 = 90; //180;
const int servo_angle4 = 90; //0;
const int servo_angle5 = 90; //110;
const int servo_angle6 = 90; //180;
const int servo_angle7 = 90; //0;
const int servo_angle8 = 90; //180;
const int servo_angle9 = 90; //0;
const int servo_angle10 = 90; //110;
...
//シリアルの読み込み＋右アーム＋ディスプレイ
void task1code(void * parameter) {
  while (true){
    if (Serial.available()) { //シリアル読みとり
      if (xSemaphoreTake(xMutex, portMAX_DELAY)) { // ミューテックスを取得
        sharedata = Serial.readStringUntil('\n');
        String data = sharedata;
        ...
        if (firstDelimiterIndex > 0){ //コマンド判定
          ...
          if (!flag){
            if (command == "redpro" || command == "bluepro" || command == "conpro"){
              ...
              display.clear();
              display.display();
              display.drawString(0,0,"ESP32 Upper.v6");
    ...
    if (xSemaphoreTake(xMutex, portMAX_DELAY)) { // ミューテックスを取得
      ...
      if (firstDelimiterIndex > 0){ //コマンド判定
        ...
        if (secondDelimiterIndex > 0) { //数値個数判定
          ...
          if (command == "UB"){
            if (firstValue == 1){
              // moveServosToTarget1(180,90,160,55,30,stepDelay); //サーボ角度制御
              // moveServosToTarget1(135,90,160,55,30,stepDelay);
              // moveServosToTarget1(135,90,180,100,90,stepDelay); 
              myservo6.write(180);
              myservo7.write(180);
              myservo8.write(180);
              myservo9.write(180);
          ...
          if (command == "RB"){
            if (firstValue == 1){
              // moveServosToTarget1(180,90,180,100,90,stepDelay);
              myservo6.write(0);
              myservo7.write(0);
              myservo8.write(0);
              myservo9.write(0);
          ...
          if (command == "L2A"){
            if (firstValue == 1){
              // moveServosToTarget1(180,90,160,55,30,stepDelay);
              // moveServosToTarget1(135,90,160,55,30,stepDelay);
              // moveServosToTarget1(135,0,10,120,70,stepDelay);
              // moveServosToTarget1(180,0,10,120,70,stepDelay);
              // moveServosToTarget1(180,90,180,90,38,stepDelay);
            }
          } 
          if (command == "MB"){
            if (firstValue == 1){
          ...
          if (command == "TAB"){
            if (firstValue == 1){
              // moveServosToTarget1(180,0,180,0,110,stepDelay);
            }
          } 
          if (command == "KS"){
            if(firstValue == 1){
...
void setup() {
  ...
  digitalWrite(SL_PIN, HIGH); //シリンダーの初期位置
