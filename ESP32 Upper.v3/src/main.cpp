    ...
    if (Serial.available()) { //シリアル読みとり
      ...
      if (firstDelimiterIndex > 0){ //コマンド判定
        ...
        if (secondDelimiterIndex > 0) { //数値個数判定
          ...
          if (command == "CB"){
            if (firstValue == 1){
              moveServosToTarget(145,90,45,120,20,stepDelay); //サーボ角度制御
            }
          }
          if (command == "TB"){
            if (firstValue == 1){
              moveServosToTarget(135,90,75,90,45,stepDelay);
            }
          }
          if (command == "XB"){
            if (firstValue == 1){
              moveServosToTarget(180,180,180,0,135,stepDelay);
            }
          }
          if (command == "KS"){
            if(firstValue == 1){
